...
class LSU extends Module {
  ...
  val wd_move = io.in.bits.src2 << ((io.in.bits.alu_result(1, 1)) << 3)

  val mask_move = mem_write_mask << ((io.in.bits.alu_result)(1, 1))
  ...
  // io.axi.WD.bits.data  := Mux(io.in.bits.alu_result(31,28)>=0xc.U,io.in.bits.src2,wd_move)
