...
class Decoder extends Module {
  val io = IO(new Bundle {
    ...
    val forwarding     = Input(UInt(CVAL.DLEN.W))
  })
  ...
  val use_forwarding_1 = (io.exu_w_addr === rs1 && rs1 =/= 0.U)
  val use_forwarding_2 = (io.exu_w_addr === rs2 && rs2 =/= 0.U)
  ...
  io.reg2.addr         := rs2
  io.csr.addr          := imm
  io.out.bits.src1     := Mux(use_forwarding_1,io.forwarding,io.reg1.data)
  io.out.bits.src2     := Mux(use_forwarding_2,io.forwarding,io.reg2.data)
