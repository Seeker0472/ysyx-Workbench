...
class REG extends Module {
  ...
  val csrs = RegInit(VecInit(Seq.fill(5)(0.U(CVAL.DLEN.W))))
  regs(0.U) := 0.U
  io.read_1 := regs(io.read_No_1)
  io.read_2 := regs(io.read_No_2)
//目前只实例化了这几个reg，使用mux来获取寄存器的数值
  val csr_r_ADDR = MuxLookup(io.csr_read_No, 0.U)(
    Seq(
      0x305.U -> 0.U,
      0x342.U -> 1.U,
      0x300.U -> 2.U,
  ...
  val csr_w_addr = MuxLookup(io.csr_write_No, 0.U)(
    Seq(
      0x305.U -> 0.U,
      0x342.U -> 1.U,
      0x300.U -> 2.U,
