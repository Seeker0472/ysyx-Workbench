...
//
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(rd)=(src1+src2)&0xffffffff);//符号拓展？？可能有问题
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd)=(src1+imm)&0xffffffff);//这两个先用64位相加，取32位？
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", addiw  , I, R(rd)=Mr(src1+imm,8));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd)=src1>>(imm&0xf));//??
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd)=src1<<(imm&0xf));
