...
//
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw   , I, R(rd)=CSR(imm&0xfff);CSR(imm&0xfff)=src1);//csrw把rd置0;csrr把rs1置0
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs   , I, R(rd)=CSR(imm&0xfff);CSR(imm&0xfff)=CSR(imm&0xfff) | src1;);//csrw把rd置0;csrr把rs1置0--printf("%x---%x--%x---",CSR(imm),src1,imm) ; printf("%d---",R(rd)) ;
