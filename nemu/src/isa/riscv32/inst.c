...
//
  ...
  //rv_zisr
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=CSR(imm&0xfff,s) | src1;);//csrw把rd置0;csrr把rs1置0
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=CSR(imm&0xfff,s) &~ src1;);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=rs1;);//csrw把rd置0;csrr把rs1置0
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=CSR(imm&0xfff,s)|rs1;);//csrw把rd置0;csrr把rs1置0
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd)=CSR(imm&0xfff,s);CSR(imm&0xfff,s)=CSR(imm&0xfff,s)&~rs1;);//csrw把rd置0;csrr把rs1置0
