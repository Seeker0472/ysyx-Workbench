...
//
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd)=s->pc+4;s->pc+=imm);//接下来要理解宏定义，然后实现J-Type的指令！！！----TODO-----
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd)=s->pc+4;s->pc=src1+imm);//接下来要理解宏定义，然后实现J-Type的指令！！！----TODO-----
